...
const CONFIG = {
  ...
  LINE_WIDTH_RANGE: [2, 50],
  SHAPE_SIZE_MIN: 20,
  SHAPE_SIZE_DIVISOR: 4,
...
const generateShapes = (width, height) => {
  ...
  let count;
  if (useGuide) {
    guideType = getRandomFromArray(CONFIG.weightedGuideTypes);
    count =
      guideType === "circle" || guideType === "square" || guideType === "spiral" || guideType === "s-curve"
        ? getRandomInt(...CONFIG.SMALL_GUIDE_COUNT)
        : Math.random() < CONFIG.HIGH_DENSITY_CHANCE
          ? getRandomInt(CONFIG.HIGH_DENSITY_MIN, CONFIG.HIGH_DENSITY_MAX)
          : getRandomInt(CONFIG.MIN_COUNT, CONFIG.MAX_COUNT);
    guidePoints = generateGuidePoints(width, height, count, guideType);
  } else {
    count = Math.random() < CONFIG.HIGH_DENSITY_CHANCE
      ? getRandomInt(CONFIG.HIGH_DENSITY_MIN, CONFIG.HIGH_DENSITY_MAX)
      : getRandomInt(CONFIG.MIN_COUNT, CONFIG.MAX_COUNT);
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    const size = getRandomInt(CONFIG.SHAPE_SIZE_MIN, maxShapeSize);
